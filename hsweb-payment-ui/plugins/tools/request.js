define(["jquery", "storejs"], function ($, storejs) {
    ...
    return {
        ...
        encodeQueryParam: function (data) {
            var query = new DSLParameterBuilder();

            function encodeValue(k, v) {
                if (k.indexOf('$LIKE') !== -1 && v.indexOf('%') === -1) return "%" + v + "%";
                if (k.indexOf('$START') !== -1) return "%" + v;
                if (k.indexOf('$END') !== -1) return v + "%";
                return v;
            }
            ...
            for (var f in data) {
                if (data[f] === "") continue;
                if (f.indexOf("@") !== -1) {
                    var nestQuery = query.nest();
                    var nest = f.split("@");

                    nestQuery.and(nest[0], encodeValue(nest[0], data[f]));
                    for (var i = 1; i < nest.length; i++) {
                        if (i % 2 === 0) {
                            nestQuery[nest[i - 1]](nest[i], encodeValue(nest[i], data[f]));
                        }
                    }
                    nestQuery.end();
                } else {
                    query.and(f, encodeValue(f, data[f]))
                }
            }
