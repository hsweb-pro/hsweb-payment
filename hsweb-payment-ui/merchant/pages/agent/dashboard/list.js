define(["echarts","request"], function (echarts,request) {
    ...
    function init(containerId) {
        require(["text!pages/agent/dashboard/list.html"], function (html) {
    ...
    function initAnalysis() {
        ...
        request.get("current-agent/sum/trading/DAY/1/7",function (e) {
            if (e.status===200){
                var key = [];
                var value = [];
                var data = e.result;
        ...
        request.get("current-agent/trading/sum-by-channel/MONTH/1/1",function (e) {
            var key = [];
            var value = [];
            var data = e.result;
            var result = [];
        ...
        request.get("current-agent/balance",function (e) {
           if (e.status===200){
               $("#balance").html((parseFloat(e.result)/100).toFixed(2));
           }
        });


        request.get("current-agent/count/order/success/DAY/1/1",function (e) {
            $("#today-success-order").html(e.result[0].total);
        });
        request.get("current-agent/count/order/DAY/1/1",function (e) {
            $("#today-all-order").html(e.result[0].total);
        });
        request.get("current-agent/count/order/requestFail/DAY/1/1",function (e) {
            $("#today-error-order").html(e.result[0].total);
        });

        request.get("current-agent/sum/amount/DAY/1",function (e) {
            $("#today-all-amount").html((parseFloat(e.result)/100).format());
        });

        request.get("current-agent/sum/amount/success/DAY/1",function (e) {
            $("#today-success-amount").html((parseFloat(e.result)/100).format());
        });

        request.get("current-agent/freeze-balance",function (e) {
            $("#current-freeze-amount").html((parseFloat(e.result)/100).format());
        });

        request.get("notice/agent",function (e) {
            var datas = e.result;
            var html = "";
        ...
        request.get("current-agent/channel/result/MONTH/1",function (e) {
           if (e.status ===200){
               var datas = e.result;
               var html = "";
