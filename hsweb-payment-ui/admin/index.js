...
//引入miniui,miniui不支持require,使用此方式引入
    importMiniui(function () {
        $(".verify-code-mg").on('click', initVerifyCode);
...
function initVerifyCode() {
    $(".verify-code-mg").attr("src", API_BASE_PATH + "verify-code?_t=" + new Date().getTime());
}
...
function initLogin() {
    ...
    bindLoginCallback();
    loginFrame.unbind("load").on("load", function () {
        bindLoginCallback();
    });

    function bindLoginCallback() {
        var iframe = loginFrame[0];
        var contentWindow = iframe.contentWindow;
        if (contentWindow) {
            contentWindow.loginCallback = function (e) {
                if (e.userType && (e.userType === 'merchant' || e.userType === 'agent')) {
                    window.top.location.href = BASE_PATH + "merchant/index.html";
                } else {
                    initAuthorize();
                    window.onLoginSuccess();
                    $("#login-frame-window").hide();
                }
            }
        }
    }
    ...
    $(".login-out").on("click", function () {
        require(["request", "message"], function (request, message) {
            message.confirm("确认退出系统?", function () {
                request.get("authorize/exit", {}, function (reponse) {
...
function initAuthorize(call) {
    require(["authorize", 'request'], function (authorize, request) {
        authorize.init(function () {
            $("#login-frame-window").hide();
            $(".username").text(authorize.user.name || authorize.user.username);
            if (call) {
                call();
            }
