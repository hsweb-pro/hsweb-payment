...
importMiniui(function () {
    ...
    require(["request", "miniui-tools", 'pages/form/designer-drag/parser', "message"], function (request, tools, FormParser, message) {
        ...
        require(["text!save.hf", "pages/form/designer-drag/components-default"], function (config) {
            ...
            var agentId = mini.getbyName("agentId");
            ...
            var agentRateGrid = mini.get("value-agent-ladder");
            ...
            function initProviderLimitEditor(row) {
                var allLimits = mini.clone(row.exactTradingLimits || {});
                var limits = allLimits[row.id] || [];
                $(limits).each(function () {
                    this.limit = (this.limit / 100).toFixed(2);
                });
                limitGrid.setData(limits);
            ...
            function initConfigLimitEditor(row, id) {
                ...
                $('.save-limit')
                    ...
                    .on('click', function () {
                        ...
                        exactTradingLimits[id] = list;
                        channelGrid.updateRow(row, row);
                        row.exactTradingLimits = exactTradingLimits;
                        mini.get('limitWindow').hide();
                    });
                mini.get('limitWindow').show();
            }

            channelGrid.getColumn("providerLimit").renderer = function (e) {
                return tools.createActionButton("限额配置", "icon-edit", function () {
                    initLimitEditor(e.record);
                });
            };

            var channelIdGrid = mini.get("channel-id-grid");
            var selectorRule = mini.getbyName("selectorRule");
            selectorRule.on("valuechanged", function (e) {
                if (e.value === 'RANDOM') {
                    channelIdGrid.selectAll();
                }
            });

            channelGrid.getColumn("selector").renderer = function (e) {
                var row = e.record;
                return tools.createActionButton("规则配置", "icon-edit", function () {
                    channelIdGrid.getColumn("limit").renderer = function (e) {
                        return tools.createActionButton("限额配置", "icon-edit", function () {
                            initConfigLimitEditor(row, e.record.id);
                        });
                    };
                    request.createQuery("channel/config")
                        .where("channel", row.channel)
                        .and("channelProvider", row.channelProvider)
                        .noPaging()
                        .exec(function (resp) {
                            channelIdGrid.setData(resp.result.data);
                            init()
                        });

                    function init() {
                        selectorRule.setValue(row.selectorRule);
                        selectorRule.doValueChanged();
                        if (row.selectorRule === 'RANGE') {
                            var config = JSON.parse(row.ruleConfig) || {};
                            channelIdGrid.findRow();
                            $(channelIdGrid.getData())
                                .each(function () {
                                    if (config.configIdList.indexOf(this.id) !== -1) {
                                        channelIdGrid.select(this);
                                    }
                                })
                        }
                        mini.get("selectorWindow").show();
                        $(".save-selector").unbind("click").on("click", function () {
                            row.selectorRule = selectorRule.getValue();
                            var idList = [];
                            $(channelIdGrid.getSelecteds()).each(function () {
                                idList.push(this.id);
                            });
                            row.ruleConfig = JSON.stringify({configIdList: idList});
                            channelGrid.updateRow(row, row);
                            mini.get("selectorWindow").hide();
                        })
                    }
                });
            };
            ...
            // channelGrid.getColumn("providerLimit").renderer = function (e) {
            //     return tools.createActionButton("限额配置", "icon-edit", function () {
            //        initProviderLimitEditor(e.record);
            //     });
            // };
            ...
            function loadData() {
                ...
                request.get("manager/merchant/" + dataId, function (response) {
                    loading.hide();
                    if (response.status === 200) {
                        ...
                        if (response.result.agentId) {
                            request.createQuery("manager/agent/no-paging")
                                .includes(["name", "id"])
                                .where("id", response.result.agentId)
                                .exec(function (agentResponse) {
                                    if (agentResponse.status === 200 && agentResponse.result.length > 0) {
                                        mini.getbyName("agentId").setValue(response.result.agentId);
                                        mini.getbyName("agentId").setText(agentResponse.result[0].name)
                                    }
                                });
                        }
                        initConfig();
            ...
            function initService() {
                $(allService).each(function () {
                    var html = $($("#service-template").html());
                    html.find(".service-name").text(this.name);
                    html.find(".enable-button").attr("id", "service-" + this.id);
                    html.find(".ip-white-list").attr("id", "service-" + this.id + "-ip-white-list");
                    $(".basic-config-container").append(html);
                })
                mini.parse()
            }
            ...
            var configs = {
                "SUPPORTED_SERVICE": {
                    setConfig: function (e) {
                        ...
                        $(conf).each(function () {
                            var serviceCheck = mini.get("service-" + this.serviceId);
                            if (serviceCheck) {
                                serviceCheck.setChecked(true);
                            }
                            var whitelist = mini.get("service-" + this.serviceId + "-ip-white-list");
                            if (whitelist) {
                                whitelist.setValue((this.ipWhiteList || []).join(";"))
                            }
                        })
                    },
                    getConfig: function () {
                        var list = [];
                        $(allService).each(function () {
                            var checked = mini.get("service-" + this.id).getValue();
                            var ipWhiteList = mini.get("service-" + this.id + "-ip-white-list").getValue();
                            if (checked === 'true') {
                                list.push({
                                    serviceId: this.id,
                ...
                "SECRET_KEY": {
                    ...
                    getConfig: function () {
                        return mini.getbyName("SECRET_KEY.value").getValue();
                    }
                },
                "SUPPORTED_CHANNEL": {
                    setConfig: function (e) {
            ...
            $(".save-basic-config").on("click", function () {
                if (checkCanSaveConfig()) {
                    var ids = ["SUPPORTED_SERVICE", "SECRET_KEY"];
                    var loading = message.loading("保存中...");
                    var total = ids.length;
                    $(ids).each(function () {
                        var id = this + "";
                        if (configs[id]) {
                            var config = configs[id].getConfig();
                            request.patch("manager/merchant/" + dataId + "/config/" + id, config, function (response) {
                                total--;
                                if (total <= 0) {
                                    loading.hide();
                                }
            ...
            $(".save").on("click", function () {
                ...
                var api = "manager/merchant/" + (dataId || "");
